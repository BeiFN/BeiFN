...
class tool {
    ...
    static move(eleNode, data, callback = () => { }, ) {
        //运动函数数据初始化
        clearInterval(eleNode.timer);//格式化：关闭存在的定时器
        eleNode.timer = setInterval(() => {//运动函数数据整理    
            var num = 0;
    ...
    static attrToendPoint(eleNode, attr, endPoint, spd = 6) {
    ...
    //事件委托          功能函数，事件对象名
    static delegate(callback, selector) {
        return evt => {
    ...
    // 账号验证函数
    static usernameRule(input, callback, regList) {
        var res = null;
        if (!regList) {
            regList = [
                {
        ...
        res = tool.valiData(input, callback, regList);
        res ? callback("") : "";
        return res;
    }
    // 密码验证函数
    static passwordRule(input, callback, regList, strengthReg) {
        var res = null;
        if (!regList) {
            regList = [
                {
        ...
        res = tool.valiData(input, callback, regList);
        res ? tool.valiStrenth(input, callback, strengthReg) : "";//密码强度提示判定
        return res;
    }
    // 邮箱验证函数
    static emailRule(input, callback, regList) {
        var res = null;
        if (!regList) {
            regList = [
                {
    ...
    // 验证函数
    static valiData(input, callback, regList) {
    ...
    // 密码强度验证
    static valiStrenth(input, callback, strengthReg) {
        var num = 0,
    ...
    static xhrGet(url, data) {
        return new Promise((resolve, reject) => {
            let xhr = new XMLHttpRequest();
            if (typeof data !== "" && data instanceof Object) {
                let _arr = [];
                for (let key in data) {
    ...
    static lazyload(selector) {
        tool.lzTimer = null;
        tool.cHeight = document.documentElement.clientHeight;
        ...
        tool.load(itemArray);
        document.addEventListener("scroll", tool.load.bind(false, itemArray));
    }
    static load(itemArray) {
        if (tool.lzTimer !== null) return;
        tool.lzTimer = setTimeout(() => {
            var scrollTop = document.body.scrollTop || document.documentElement.scrollTop;
            var min = scrollTop + tool.cHeight;
    ...
    // jsonp跨域
    static jsonp(url, cb_key = "callback", data = {}) {
