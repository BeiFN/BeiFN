/**
 * @Validate   正则表单验证(京东正则)
 * 参数:
 *      @selector   form元素
 *      @options   自定义正则对象,结构：{"username": [{reg,bool,tip}],"strength":{"reg":[],"tip":[],"state":[class名]}}
 * 内部元素属性名：
 *      @v_type     输入框类型
 *      @v_tip      提示框类型
 *      @v_strength 是否进行密码强度判断
 * 配置class名：
 *      @success    输入框、提示框验证成功状态
 *      @error      输入框、提示框验证失败状态
 *      @lower      密码提示框密码强度低
 *      @middle     密码提示框密码强度中
 *      @high       密码提示框密码强度高
...
class Validate {
    constructor(selector, options) {
        ...
        this.iptList = [].slice.call(this.box.querySelectorAll("input[v_type]"));
        this.tipBoxList = [].slice.call(this.box.querySelectorAll("[v_tip]"));
        this.binEvent();
    }
    // 事件绑定
    binEvent() {
    ...
    init(ipt) {
        this.ipt = ipt;                                     // 输入框
        this.type = this.ipt.getAttribute("v_type");        // 类型
        ...
        this.tipBoxList.forEach(tipBox => {
            if (tipBox.getAttribute("v_tip") === this.type) {
                this.tipBox = tipBox;
            }
        })
        ...
        if (this.type === "password" && this.ipt.getAttribute("v_strength") && this.state === "success") {
