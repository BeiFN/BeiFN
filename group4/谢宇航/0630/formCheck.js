;+function (window) {
    ...
    function btnHandler(ele) {
    	console.log(ele.parentNode.querySelectorAll("input"));
    	var inputList = ele.parentNode.querySelectorAll("input");
    	var flag = true;
    	for(var i = 0; i < inputList.length; i++){
    		if(/error/g.test(inputList[i].className) || inputList[i].value.length === 0){
    			flag = false;
    			addValidateState("error",inputList[i]);
    		}
    	}
    	flag === true ? alert("注册成功！"): alert("注册失败！");
    }
