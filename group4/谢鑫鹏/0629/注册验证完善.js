...
;+function(window){
    ...
    var strategyList = {
        ...
        "username" : {
            reg : /^[\u4e00-\u9f5aa-z0-9]{4,20}$/,
            title : "中文，英文，数字，-，_ 组成的4~20位字符,不允许为纯数字",
            flag : false
        },
        "password" : {
            reg : /^[\!\@\#\$\%\^\&\*\(\)0-9a-z_\-]{6,}$/i,
            title : "密码的长度应该在6位之上，允许!@#$%^&*()+数字+字母",
            flag : false
        },
    }
    ...
    function validateText(id){
        ...
        if(strategyList[id].reg.test(value) && this.getAttribute("v-pureNumber")){
            validatePureNumber(value, this, this.getAttribute("v-pureNumber") === "true" ? true : false, id);
        }
    }
    /**
     * 验证字符串是否符合规则
    ...
    function addValidateState(state, ele, id){
        ...
        if(state === "success"){
            ...
            newEle.parentNode.children[0].style.color = "green";
        }else{
            newEle.innerHTML = strategyList[id].title;
            strategyList[id].flag = false;
        }
    }
    /**
     * 验证密码强度
    ...
    function validateStrength(value,ele){
        ...
        var strengthText = ele.parentNode.children[0];
        switch(score){
            case 0 :
                addValidateState("error", ele, id);
            case 1 : 
                addStrengthState("low" , ele);
                strengthText.innerHTML = "成功（密码强度弱）";
                break;
            case 2 : 
                addStrengthState("middle" , ele);
                strengthText.innerHTML = "成功（密码强度中）";
                break;
            case 3 : 
                addStrengthState("high" , ele);
                strengthText.innerHTML = "成功（密码强度强）";
                break;
            default :
                addStrengthState("low" , ele);
        }
     ...
     * 验证是否为纯数字函数
     */
    function validatePureNumber(value, ele, bool, id){
        if(bool){
            if(/^\d+$/.test(value)){
                addValidateState("success", ele, id);
            }else{
    ...
    btn.onclick = function(){
        var flag = true;
        for(ele in strategyList){
            flag = flag && strategyList[ele].flag;
        }
        if(flag === true){
            alert("注册成功");
        }
