...
Magnifier.prototype.init = function(){
    ...
    this.smallImgWrap.addEventListener("mouseleave", this.hide.bind(this));
    this.smallImgWrap.addEventListener("mousemove",this.handleMousemove.bind(this));
    for(var i = 0, ele; ele = this.imgBtns[i]; i ++){
        ele.addEventListener("click",this.changeImg.bind(this, ele, i));
    }
}
// 鼠标移入时显示
...
// 鼠标移动处理
Magnifier.prototype.handleMousemove = function(evt){
...
Magnifier.prototype.boundary = function(x, y){
    x = x < 0 ? 0 : x;
    var xMax = this.smallImgWrapSize.width - this.smallCubeSize.width;
    x = x > xMax ? xMax : x; 
    y = y < 0 ? 0 : y;
    var yMax = this.smallImgWrapSize.height - this.smallCubeSize.height;
    y = y > yMax ? yMax : y;
    return {
        x : x,
        y : y
...
// 根据比例计算大图位置
Magnifier.prototype.getBigPosition = function(small){
    // 获取总宽度、高度
    var smallTotalWidth = this.smallImgWrapSize.width - this.smallCubeSize.width;
    var smallTotalHeight = this.smallImgWrapSize.height - this.smallCubeSize.height;
    var bigTotalWidth = this.bigImgSize.width - this.bigImgWrapSize.width;
    var bigTotalHeight = this.bigImgSize.height - this.bigImgWrapSize.height;
    // 通过比例计算位置
    var bigPositionX = -(small.x / smallTotalWidth) * bigTotalWidth;
    var bigPositionY = -(small.y / smallTotalHeight) * bigTotalHeight;
    return {
        x : bigPositionX,
        y : bigPositionY
    }
