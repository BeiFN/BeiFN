...
class WaterFall{
      ...
      async init(){
          ...
          on(window, "scroll" ,async ()=>{
                ...
                if(!this.loading && scrollTop + this.cHeight >= this.containerHeight - 300){
                   ...
                   this.next_start = Load.nextStart;
                    this.render(res);
                   //this.wrapper.innerHTML += this.render(res);    //  CHANGE  ONE
                   this.sort();
                   // 加载结束要归零;
                   this.loading = false;
                }
          ...
          this.next_start = Load.nextStart;
          this.render(res);
          //this.wrapper.innerHTML += this.render(res);    //  CHANGE TWO
          this.sort();
      }
...
class anotherWaterFall extends WaterFall{
      async init(){
            this.template    = $("#template");
            this.wrapper     = $(".wrapper");
            this.container   = $(".container");
      ...
      render(list){
            ...
            list.forEach(function(item){
                  // html +=`<p>${item.album.name}</p>`;
                  //html +=`<p>${item.msg}</p>`;
                  html +=`<p> 上传时间：${item.add_datetime_pretty}    ${item.add_datetime}</p> `;
            })
            return html;
      }
