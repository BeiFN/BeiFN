...
####### 第五周
##问题39class的默认参数怎么设置与使用呢？ 是在constructor方法小括号中默认了参数后，，再在函数内部进行赋值，是这样操作的吗？
```javascript
constructor({small_wrapper = ".small-img"}={}）{
    this.small_wrapper = static $(small_wrapper); 
}
...
## 问题40：需要讲一个完整的class案例，轮播图、放大镜之类的。
>见飞机大战


## 问题41: 咋一看代码，看似都明白，细细思量，似懂非懂，自己写起来，啥都忘了，满脑袋空白，一脸懵逼。不看一眼代码，或者三四五六眼，根本想不起来，该怎么改善？
...
## 问题42:静态方法和属性可以在别的类中被使用,但是飞机大战核心部分里的创建元素函数只在Core类中使用,是出于什么原因设置成静态的呢?什么时候适合用静态方法和静态属性呢?

* 为了教你们用这个东西。
* 在类之间进行耦合设计的时候。

## 问题43: 飞机大战进入游戏前，飞机往右飞的加载图 怎么使用网络图片设置？

* 用数组承载图片的路径，在飞的时候遍历数组就可以了;

## 问题44：模板引擎库的使用，弄不明白，需要一个案例。

* 案例很快遍地都是;


## 问题45: 智哥,你用的搜索文件的快捷方式是怎么弄得,就是每次桌面右下角出现的那个窗口?看起来好优雅哦

* listary
  
[listary](https://www.listary.com/)

## 问题46: 智哥,js是基于对象而不是面向对象的这种说法该怎么理解?
...
## 问题47: MySQL创键数据库时，连接名是用来干嘛的？只是个单纯分组吗？
...
#问题48： 数据库遍历while循环中的=号怎么理解？ 感觉赋值 和 判断等于都不太像。
...
#问题49： 可以一次给表添加(插入)多条数据(记录)吗？如果可以的话，该怎么添加呢？
...
#问题50:  php解析器位于什么地方,是和js解析器一样在浏览器里吗?还是在服务器里?
...
## 问题51: 后台服务器可以获取前端传过去的key值吗？ 后台写的固定key值'callback'，那封装中的cb_fild是不是可以省略？
...
##问题52 能否通过传入具名函数，获取到它的函数名字符串，如果可以这样的话，是不是可以省去设置全局函数调用回调函数的步骤？

* 封装原则：高内聚 ， 低耦合 ;
* 该方式 严重耦合全局函数，封装的功能不独立。不建议这样使用
...
##问题53 promise连缀那里听得不太明白，具体是咋运用，请智哥再讲一下。
```javascript
...
##问题54  实际开发中，如果是和后台联调，服务器不在本机时，使用ajax请求时，如何让本地代码和后台服务器同源？ 

 * nginx 
 

##问题55  智哥，书上说script可以共享函数，是不是说就不用自己再声明一个全局函数以供外部调用了啊 ??
...
## 问题56  智哥 能讲下TOKEN 与cookie 的关系，domain与path 的关系吗?
...
## 问题57  免登陆案例中，为什么修改了utils里面的代码，但在浏览器控制台sources里面，还是之前的utils代码，重启浏览器和服务器都一样，电脑重启后代码正常，是浏览器缓存原因吗？该怎么处理？
