...
class Bullet{
    ...
    createBullet(){
        ...
        let left = Plane.x +Plane.plane_size.width/2 - (Bullet.bullet_size ?
            Bullet.bullet_size.width/2 : ele.offsetWidth/2);
        let top = Plane.y;
...
class Enemy{
    constructor(){
        ...
        Enemy.enemy_timer ? "" : Enemy.enemy_timer = setInterval(()=>{
            this.enemyMove();
            //碰撞检测
            this.collosionDetection();
        },50);

        Enemy.enemy_list ? "" : Enemy.enemy_list = [];
        //敌机  className ，speed，hp，die  className   不同
