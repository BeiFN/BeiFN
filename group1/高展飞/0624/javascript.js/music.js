...
function handlerVolumeChangeMove(evt){
    var e = evt || event ;
    //根据鼠标位置，设置当前元素位置;根据点击位置计算元素正确位置
...
function changeVolumeIcon(prop){
    if(prop === 0){
        //静音图标
        hron.style.backgroundImage = "url("+GLOBAL.volume.mute+")"
    }else if(prop >0 && prop <33){
        hron.style.backgroundImage = "url("+GLOBAL.volume.low+")"
    }else if(prop >=33 && prop <66){
        hron.style.backgroundImage = "url("+GLOBAL.volume.middle+")"
    }else {
        hron.style.backgroundImage = "url("+GLOBAL.volume.hight+")"
    }
}
