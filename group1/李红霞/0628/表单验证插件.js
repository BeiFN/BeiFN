...
// console.log(form, email, username, password, btn)
...
function handlerValid(evt){
    ...
    //判断是否可以为纯数字
    if(target.getAttribute("pureNumber") == "false"){
        /^\d+$/.test(target.value)?console.log("error"):"";
    }
}
//验证密码强度
function validStrength(target){
    if(target.getAttribute("v-strength") !== "true"){
        return false;
    }
...
function validText(target){
    ...
    if(regList[type].reg.test(value)){
        ...
        createTipElement("success", target);  
    }else{
        addstyle(target, "fail");
        createTipElement("fail", target);        
    }
}

function createTipElement(state, target){
    var sp = document.createElement("span");
    sp.innerHTML = "input " + state +"! <br>";
