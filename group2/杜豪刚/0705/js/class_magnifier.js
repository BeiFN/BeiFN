...
//  获取utils的静态方法
let { getSize,$,removeClassName,delegation,getAbsPosition } = Utils ;
// console.log(getSize,$,removeClassName,delegation,getAbsPosition);

class Magnifier{

    // constructor 放属性
    constructor( {small_wrapper = ".small-img",small_img = ".small-img img",small_cube = ".cube",
        big_wrapper = ".big-img", big_img = ".big-img img",btn_wrapper =".img-btn" } = {} ){
        ...
        this.small_wrapper = $(small_wrapper);           //small外包围
        this.small_img     = $(small_img);       //small图片
        this.small_cube    = $(small_cube);                //cube区域
        this.big_wrapper   = $(big_wrapper);             //big外包围
        this.big_img       = $(big_img);         //big图片
        this.btn_wrapper   = $(btn_wrapper);             //img-btn外包围
    ...
    // 移动cube
    move(cube_position,big_position){
        // 小图移动的位置
   ...
   mouseMove (evt){
        ...
        var big_position  = this.getBigPosition(cube_position.x,cube_position.y);                 //确定大图的位置
