...
class WaterFall {
    ...
    render(){
        ...
        for(let i = 0 ; i < data.length ; i ++){
            ...
            html += `<div class="box">
                             <div class="box-img" style =" height:${scaleHeight}px" data-img = "${data[i].photo.path}">
                                    <img style =" height:${scaleHeight}px" src="${this.load_url}" alt="">
                            </div>
    ...
    getImgAll(){
        ...
        this.imgs.forEach(img=>{
            img.children[0].setAttribute("data-top",img.offsetTop);
        });
    }
    showImg(scrollTop){
        this.imgs.forEach( img => {
            let img_top = img.children[0].getAttribute("data-top");
            if(scrollTop + this.cHeight >= img_top - 300 || img_top < this.cHeight){
                  // console.log(img);
                  let src = img.children[0].getAttribute("data-img");
