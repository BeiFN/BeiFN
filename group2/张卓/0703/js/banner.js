...
//分页的点击事件
Banner.prototype.fenyeClick = function(evt){
    var e = evt||event;
    var target = e.target || e.srcEvent;
    if(target!==this.fenye){
        //点击的都是分页内的子元素
        var arr = [].slice.call(this.fenye.children)
        if(arr.indexOf(target)>-1){
            this.toIndex(arr.indexOf(target))
        }
        
        // var arr = [].slice.call(target.parentNode.children)
        // if(arr.indexOf(target)>-1){
        //       this.toIndex(arr.indexOf(target));
        // }
    }
...
//改变分页栏的布局
Banner.prototype.fyStyleChange = function(){
    ...
    var html = ""
    for(var i = 0 ; i < this.fenyeCount; i++){
        if(i == 0){
            //默认首个元素为active的
...
//动画布局改变
Banner.prototype.animationChange = function () {
...
Banner.prototype.slideStyleChange = function () {
