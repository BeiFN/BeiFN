...
class Magnifier {
    ...
    handlerImgBtnClick(evt, ele) {
        //ele
        let target = evt.target || evt.srcElement
        for (let i = 0, btn; btn = this.imgBtns[i]; i++) {
    ...
    handlerMove(evt) {
        let e = evt || event;
        let x = e.pageX - this.smallcube.offsetWidth / 2 - this.abs.width;
        let y = e.pageY - this.smallcube.offsetHeight / 2 - this.abs.height;
        //卡住边界的函数  将传入的x和y的值进行改变 在给下面的物体进行运动
        let smallBoundary = this.boundary(x, y)
        let bigPosition = this.bigImgPosition(smallBoundary, e)
    ...
    //大图的位置
    bigImgPosition(smallBoundary, e) {
        let x = (smallBoundary.x / (this.smallImgWrapper.size.width - this.smallcube.size.width) * (this.bigImg.size.width - this.bigImgWrapper.size.width))
        let y = (smallBoundary.y / (this.smallImgWrapper.size.height - this.smallcube.size.height) * (this.bigImg.size.height - this.bigImgWrapper.size.height))
        // console.log(x,y)
        return {
            x: x,
            y: y
    ...
    //移动
    move(smallBoundary, bigPosition) {
        //方块移动
    ...
    //事件委托函数
    static delegation(handlEvt, selector) {
        return function (evt) {
            let e = evt || event;
            let target = e.target || e.srcElement
            let _tampTarget = target
            //添加家族
            let targetFamily = [];
            let selectList = this.querySelectorAll(selector)
            //添加家族元素
            while (true) {
            ...
            for (let i = 0, ele; ele = selectList[i++];) {
