...
class Magnifier{
    constructor({small_img=".small-img",smallimg=".small-img img",cube=".cube",big_img=".big-img",bigimg=".big-img img",img_btn=".img-btn",}={}){

        this.small_wrapper = Common.$(small_img);
        console.log(this.small_wrapper);
        this.small_img     = Common.$(smallimg);
        this.small_cube    = Common.$(cube);
        this.big_wrapper   = Common.$(big_img);
        this.big_img       = Common.$(bigimg)
        this.img_btn_wrapper = Common.$(img_btn);
        ...
        this.small_wrapper_position = Common.getAbsPostion(this.small_wrapper);
        this.small_cube_size        = Common.getSize(this.small_cube);
        this.small_wrapper_size     = Common.getSize(this.small_wrapper);
        this.big_img_size           = Common.getSize(this.big_img);
        this.big_wrapper_size       = Common.getSize(this.big_wrapper);
        this.init();   //调用核心函数
    ...
    init (){
        ...
        this.img_btn_wrapper.addEventListener("click",Common.delegation(this.changeImg.bind(this),".img-box"));
...
//移动
    move(cube_postion,big_img_position){
...
//4.变换图片
    changeImg(evt,ele){
        ...
        for(var i = 0,btn;btn = this.img_btns[i++];){
            Common.removeClassName(btn,"active");
        }
...
class Common{
        ...
        if(dom.offsetParent === document.body){
            ...
            var pos = Common.getAbsPostion(dom.offsetParent)
            return{
