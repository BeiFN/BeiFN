 ...
 *  Author:EMB
 *  Date:2019.7.12      
 *  Now Version:V:D-0.0.6
 * 
 *  Description:
 * ----------------------------↓
 *      Function: 
 *
 *      *随机数的生成 randomNumber(min [最小范围], max[最大范围]) ;
 *      *AJAX 请求的封装 
 *              ajax(url:请求路径, [option]
 *              {type:请求方法,
 *              async: 是否异步,
 *              jsonp =JSONP参数,
 *              jsonpCallback:请求的JSONP公共回调,
 *              data:请求数据对象,
 *              timeout:超时设置,
 *              dataType:处理回传数据(不处理)|text(字符串)|json(对象),
 *              error :error=>console.log("Request Error:"+error), 描述 请求的错误回调,
 *              success : data=>console.log("Request Success ",data) 描述 请求的成功回调
 *              });
 *      
 *      -------------------↓
 *  
 ...
 *      Update Description:
 *      ------------------↓
 *      v 0.0.1  2019.6.12  基础 对象类库 随机颜色，随机字符，Timer 倒数计时器 ，及（本人认为的）万能的时间处理方法 getTime ，padInt补全
 ...
 *      v 0.0.5 2019.7.11 加入ajax 实现基本的 JQ get post 和实现ES6 的Promise Then。
 *      v 0.0.6 2019.7.12 加入Cookie 操作的基本兼容方法
 *      -------------------↑
 * 
 *      Version Initials:
 *      ------------------↓
